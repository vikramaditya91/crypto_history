...
class PrimitiveCoinHistoryObtainer:
    ...
    async def get_historical_data_relevant_coins_from_base_and_reference_assets(
            self,
            base_assets: List,
    ...
    async def _get_historical_data_relevant_coins(self, tickers_to_capture: List[tuple]) -> Dict:
        """
...
class DataFrameOperations:
    ...
    @staticmethod
    def drop_unnecessary_columns_from_df(df, necessary_columns: List) -> DataFrame:
        """
    ...
    async def get_compatible_df(self,
                                standard_example: List,
                                ticker_history: DataFrame) -> DataFrame:
        """
        ...
        padded_df = await self.pad_extra_rows_if_necessary(standard_example,
                                                           history_df)
        return padded_df
    ...
    async def pad_extra_rows_if_necessary(self,
                                          standard_example: List,
        ...
        Args:
            standard_example (DataFrame): standard example on which it is based
...
class PrimitiveDataArrayOperations:
    ...
    def __init__(self,
                 ...
                 limit: int):
        """
        Initializes the DataContainerOperations which is the user end-point for the basic data building
        Args:
            exchange_factory (StockMarketFactory): factory of the exchange
            base_assets (List): list of base coins to be accumulated
            reference_assets (List): list of reference/quote-against coins to be accumulated
    ...
    async def populate_container(self,
                                 data_container: xr.DataArray,
                                 coord_dimension_dataclass) -> xr.DataArray:
        """
        ...
        for (base_asset, reference_asset), ticker_raw_history in historical_data.items():
            try:
                history_df = await self.dataframe_operations.get_compatible_df(standard_example_df,
                                                                               ticker_raw_history)
                ...
                history_df = self.dataframe_operations.drop_unnecessary_columns_from_df(
                    history_df,
                    coord_dimension_dataclass.ohlcv_fields
                )
                history_df = self.append_column_to_df(history_df, "weight", self.interval)
